...
class CompanyTestCase(unittest.TestCase):
  ...
  def test_list_user_Company(self):
    ...
    company = Company(name = "Name", create_by= user1, 
                  email="asdd@asdasd.com", atach_users = [user1, user2])
    company.put()

    self.assertEqual(1, len(Company.all().fetch(10)))
    self.assertEqual(2, len(Company.all().fetch(1)[0].atach_users))
    self.assertEqual(user1, Company.all().fetch(1)[0].atach_users[0])
    self.assertEqual(user2, Company.all().fetch(1)[0].atach_users[1])
